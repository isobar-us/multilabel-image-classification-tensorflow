...
Inception V3) for each image. We train a sigmoid layer on top of this
representation. If the sigmoid layer contains N labels, this corresponds
...
# parses the given lst file line by line and returns results as a json list
# where each line is represented as key being image name and value being
# binary label flags (1 means image has label and 0 means it doesn't)
def parse_lst_file(lst_path):
    parsed_results = []
    with open(lst_path, encoding='utf-8') as train_lst:
        for cnt, line in enumerate(train_lst):
            line_values = line.rstrip().split('\t')
            image_name = line_values[-1]
            line_labels = line_values[1:-1]
            line_labels = [float(i) for i in line_labels]
            
            if len(line_labels) != FLAGS.num_classes:
                raise RuntimeError('Number of classes in dataset not macthing {}'
                                 .format(FLAGS.num_classes))
            
            parsed_results.append({image_name : line_labels})
...
def create_image_lists(image_dir, testing_percentage, validation_percentage):
    ...
    parsed_lst_train = parse_lst_file(image_dir + '/dataset_train.lst')

    # load the validation set
    parsed_lst_validation = parse_lst_file(image_dir + '/dataset_test.lst')

    validation_size = int(len(parsed_lst_validation)*(100-testing_percentage)/100)

    print("Validation size {}".format(validation_size))

    training_images = parsed_lst_train
    validation_images = parsed_lst_validation[0:validation_size]
    testing_images = parsed_lst_validation[validation_size:-1]
...
def get_image_path(image_lists, label_name, index, image_dir, category):
    ...
    image_name = list(category_list[mod_index].keys())[0]
...
def run_bottleneck_on_image(sess, image_data, image_data_tensor,
    ...
    Args:
      ...
      bottleneck_tensor: Layer before the final sigmoid.
...
def get_random_cached_bottlenecks(sess, image_lists, how_many, category,
    ...
    if how_many >= 0:
        ...
        for unused_i in range(how_many):
            image_index = random.randrange(len(images_by_category))
            image_labels = images_by_category[image_index]
            image_name = list(image_labels)[0]
            label_flags = image_labels[image_name]

            image_path = get_image_path(image_lists, "images", image_index,
                                        image_dir, category)

            bottleneck = get_or_create_bottleneck(
                sess, image_lists, "images", image_index, image_dir, category,
                bottleneck_dir, jpeg_data_tensor, decoded_image_tensor,
                resized_input_tensor, bottleneck_tensor, module_name)

            bottlenecks.append(bottleneck)
            ground_truths.append(label_flags)
        ...
        for image_index, image_by_category in enumerate(images_by_category):
            ...
            label_flags = image_by_category[image_name]
...
def add_final_retrain_ops(class_count, final_tensor_name, bottleneck_tensor,
    ...
    """Adds a new sigmoid and fully-connected layer for training and eval.

    We need to retrain the top layer to identify our new classes, so this function
    adds the right operations to the graph, along with some variables to hold the
    weights, and then sets up all the gradients for the backward pass.

    The set up for the sigmoid and fully-connected layers is based on:
...
def run_final_eval(train_session, module_spec, class_count, image_lists,
    ...
    if FLAGS.print_misclassified_test_images:
        ...
        for i, test_filename in enumerate(test_filenames):
            if not np.array_equal(predictions[i], test_ground_truth[i]):
                tf.logging.info("Test file {} predicted as {} as opposed to {}"
                                .format(test_filename, predictions[i], 
                                        test_ground_truth[i]))
